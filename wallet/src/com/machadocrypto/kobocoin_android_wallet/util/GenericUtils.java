...
{
	public static final BigInteger ONE_KOBO = new BigInteger("1000000", 10);
	public static final BigInteger ONE_MKOBO = new BigInteger("1000", 10);
	public static final BigInteger ONE_UKOBO = new BigInteger("1", 10);

	private static final int ONE_KOBO_INT = ONE_KOBO.intValue();
	private static final int ONE_MKOBO_INT = ONE_MKOBO.intValue();
	private static final int ONE_UKOBO_INT = ONE_UKOBO.intValue();
			...
			final int precision, final int shift)
	{
		...
		if (shift == 0)
		{
			...
			final long coins = absValue / ONE_KOBO_INT;
			final int satoshis = (int) (absValue % ONE_KOBO_INT);
		...
		else if (shift == 3)
		{
			...
			final long absValue = Math.abs(longValue);
			final long coins = absValue / ONE_MKOBO_INT;
			final int satoshis = (int) (absValue % ONE_MKOBO_INT);
		...
		else if (shift == 6)
		{
			...
			final long absValue = Math.abs(longValue);
			final long coins = absValue / ONE_UKOBO_INT;
			final int satoshis = (int) (absValue % ONE_UKOBO_INT);
	...
	{
		return formatValue(value, Constants.KOBO_MAX_PRECISION, 0);
	}
