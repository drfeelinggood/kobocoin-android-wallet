...
public class Configuration
{
	...
	public static final String PREFS_KEY_KOBO_PRECISION = "kobo_precision";
	...
	private static final int PREFS_DEFAULT_KOBO_SHIFT = 0;
	private static final int PREFS_DEFAULT_KOBO_PRECISION = 2;
	...
	public boolean hasKOBOPrecision()
	{
		return prefs.contains(PREFS_KEY_KOBO_PRECISION);
	}

	public int getKOBOPrecision()
	{
		final String precision = prefs.getString(PREFS_KEY_KOBO_PRECISION, null);
		if (precision != null)
			return precision.charAt(0) - '0';
		else
			return PREFS_DEFAULT_KOBO_PRECISION;
	}

	public int getKOBOMaxPrecision()
	{
		final int KOBOShift = getKOBOShift();

		if (KOBOShift == 0)
			return Constants.KOBO_MAX_PRECISION;
		else if (KOBOShift == 3)
			return Constants.MKOBO_MAX_PRECISION;
		else
			return Constants.UKOBO_MAX_PRECISION;
	}

	public int getKOBOShift()
	{
		final String precision = prefs.getString(PREFS_KEY_KOBO_PRECISION, null);
		if (precision != null)
			return precision.length() == 3 ? precision.charAt(2) - '0' : 0;
		else
			return PREFS_DEFAULT_KOBO_SHIFT;
	}

	public String getKOBOPrefix()
	{
		final int KOBOShift = getKOBOShift();

		if (KOBOShift == 0)
			return Constants.CURRENCY_CODE_KOBO;
		else if (KOBOShift == 3)
			return Constants.CURRENCY_CODE_MKOBO;
		else
			return Constants.CURRENCY_CODE_UKOBO;
	}
